...
#
'''
python3 train.py --img-size 640 --batch-size 16 --epochs 200 --data ./data/fund.yaml --cfg ./models/score/yolov5x.yaml --weights weights/yolov5x.pt
...
suffix = "cdla3_fund3"
last = wdir + 'last_' + suffix + '.pt'
best = wdir + 'best_' + suffix + '.pt'
...
def train(hyp):
    ...
    # Image sizes
    gs = int(max(model.stride))  # grid size (max stride)python3 train.py --img-size 640 --batch-size 8 --epochs 200 --data ./data/layout.yaml --cfg ./models/score/yolov5x.yaml --weights weights/yolov5x.pt
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        if ckpt['optimizer'] is not None:
            ...
            best_fitness = 0 # 设置为0，避免无法存储best
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', action='store_true', help='resume training from last.pt') # 是否继续训练的标识。
    ...
    parser.add_argument('--save_suffix', default='', help='renames results.txt to results_name.txt if supplied')
